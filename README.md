...
>
> &middot; **TR-300 `v3.16` &rarr; `v3.17`, the stability + single-install line.** The flagship Rust machine-report CLI ([`qube-machine-report`](https://github.com/QubeTX/qube-machine-report)) took a `v3.16.0` seven-PR cross-platform hardening pass: Windows + Linux collector accuracy, output / build robustness, install-update robustness, a self-update reliability deep-dive, and a dedicated test-coverage sweep.
>
> **`v3.17.0` adds migrate-cleanup:** installing through a new channel (Windows installer, cargo, bare binary) now detects and removes any prior install of a different kind, so the machine carries exactly one TR-300 at a time.
>
> &middot; **shaughv-code `v0.16.0`, now a Mistral-capable marketplace.** A new Mistral AI skill makes an agent fully fluent in the Mistral API: OCR, audio transcription + text-to-speech, chat / tools / structured output, embeddings, classifiers, batch, fine-tuning, and agents, with dependency-optional Python runners. Sits on the `v0.13` &rarr; `v0.15` run: a `usage-statusline` skill with a zero-dep cross-platform installer, a `crystal-upscaler` image skill, and the `shaughv-cdn` consumer skill rebuilt manifest-driven so it self-heals off `cdn.shaughv.com/tree.json`.
>
