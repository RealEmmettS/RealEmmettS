...
- 🥫 **Magic Pantry** – just landed the Replit → Supabase migration and Phase H account self-service: forgot / reset / change password, change username, signup-confirmation flow with resend, and `magicpantry://` deep links wired end-to-end for App Store launch. Postgres with private-schema RLS helpers, Realtime, and Anthropic + Firecrawl edge functions for categorization / recipe generation / URL recipe import / account deletion
