...
>
> &middot; **Magic Pantry, offline queue.** Mutations queue behind a new `useStoreSyncStatus` hook on `magicPantry_2026` and replay cleanly on reconnect; sync state surfaces in `StoreDetailScreen` / `StoreListsScreen` / `ListItem` with theme tokens for offline / syncing / synced.
>
> Sits on top of the **2.0 / 2.0.1 store-ready prep**: EAS wired into `app.json`, Supabase magic-link sign-in via `magicpantry://` deep links, a Neon &rarr; Supabase migration runner with backups, an AI model policy in `AGENTS.md` (Haiku 4.5 for item helpers, Sonnet 4.6 for generation), and a `.agents/demo-test-command.md` pinning the PowerShell-backgrounded `npx expo start` workflow for live Expo Go demos.
>
> &middot; **TR-300, May-27 security hardening triplet.** Three Codex-authored PRs on [`qube-machine-report`](https://github.com/QubeTX/qube-machine-report) landing the same morning.
>
> **PR #1:** Windows updater TOCTOU close, `unique_update_temp_path()` (VERSION + nanos timestamp + PID + sequence) and `OpenOptions::create_new(true)` so a pre-planted symlink can't redirect or truncate.
>
> **PR #2:** fast-mode GPU helpers resolved via absolute system paths (`C:\Windows\System32\reg.exe`; `/usr/bin/lspci` then `/bin/lspci` etc.; `/usr/sbin/ioreg` + `/usr/sbin/sysctl`), closing the PATH-hijack vector under `--fast` auto-run.
>
> **PR #3:** markdown auto-save against privileged clobber, `is_elevated` guard + `create_new(true)` + PID/nanos filename so the path isn't pre-creatable. Each PR ran `cargo fmt` / `clippy -D warnings` / `cargo test` clean.
>
> &middot; **Day-job, 12-task internal-tools sprint.** Twelve shipped tasks across an internal monthly-reporting tool and an internal scorecards dashboard.
>
> **Reporting tool:** a `Regenerate Report` flow (versioned `report_narrative_snapshots` + 30s undo + `View History` restore-to-version + chat-aware re-synthesis), embedded-chat persona lock, PM questionnaire wired into the generated PDF, plus a PDF render cleanup pass (broken PM lookup, phantom photo rectangles, chat panel layout, `Clear Chat` destructive modal).
>
> **Scorecards dashboard:** a 6-chart Visual Snapshot section paired with the coding agent (contingency gauge, billed donut, budget-vs-actual bar group, GM% sparkline, monthly-cost performance bar, cumulative-cash area trend) backed by a new `GET /api/project-scorecard-history` endpoint.
>
> **Stack:** Python / FastAPI, TypeScript + React, SQL, Azure Container Apps with named-revision rollouts.
...
[`shaughv-code`](https://github.com/RealEmmettS/shaughv-code) is the SHAUGHV Claude Code plugin marketplace at v0.7.0 after a five-release sprint: bundling `critical-thinking`, `openai-audio`, `pretext`, `perplexity-search`, `quiver-ai`, `shaughv-animated-brandmark`, `shaughv-design`, `/human-changelog`, `shaughv-cdn`, `naming-conventions`, and paired `gcs-storage` + `shaughv-gcs-storage`, plus two MCP server bundles (`@remotion/mcp` powering `/shaughv-code:create-video`, and `craft-docs` over Streamable HTTP + OAuth).
...
|        | **May** | **TR-300:** v3.14.x preflight chain, v3.15.0 / v3.15.1 four-installer Windows distribution, v3.15.2 cross-platform Opus 4.7 audit + SHA256 installer verification, v3.15.3 audit close (F17 / F20 / F22), May-27 security hardening triplet (updater + fast-mode GPU helpers + markdown auto-save). <br><br> **Magic Pantry:** Phase H + Phases I-L (smart entry, recipes library, substitutions, sort modes, 5 new edge functions), 2.0 / 2.0.1 store-ready prep (EAS, magic-link sign-in, Neon &rarr; Supabase migration runner, AI model policy), offline queued store actions. <br><br> **SHAUGHV brand:** `shaughv-cdn` launch (91 assets to Cloudflare R2), `shaughv-code` v0.2.0 &rarr; v0.7.0 marketplace sprint, `italy-trip-video` Remotion 50th-birthday slideshow, `shaughv_vintage` refresh + project-level Claude Code automation. <br><br> **Web tools:** `qrgen` SHAUGHV product surface rebuild with time-of-day dual palette + QR outline tracer, `realtime2_test` OpenAI Realtime API prototype, QorkMe SHAUGHV vintage palette migration. <br><br> **Other:** shaughvOS v1.20, day-job 12-task sprint on an internal monthly-reporting tool + scorecards dashboard (Regenerate Report flow with versioned snapshots + undo + history, 6-chart Visual Snapshot data-viz layer, internal task-queue MCP `update_task` multi-operator edit). |
...
- **shaughv-code v0.2.0 &rarr; v0.7.0.** Five-release sprint on the SHAUGHV Claude Code plugin marketplace.
...
- **Magic Pantry 2.0 / 2.0.1 store-ready prep.** App Store submission window.
  - **EAS wired** into `app.json` (`extra.eas.projectId` + `owner = "realemmetts"`) so cloud builds + store submissions can run.
...
- **Day-job, internal-tools cycle.** Python / FastAPI + TypeScript + React + SQL + Azure Container Apps.
  - **Monthly-reporting tool:** Regenerate Report flow (versioned `report_narrative_snapshots`, 30s undo, restore-to-version history, chat-aware re-synthesis); embedded-chat persona lock; PM questionnaire wired into the PDF; PDF render fixes (broken PM lookup, phantom photo rectangles + border / mat treatment, chat panel layout, `Clear Chat` destructive modal); date-answer validation tightened; `questionnaire_seed_failed` audit event for KQL observability.
  - **Scorecards dashboard:** 6-chart Visual Snapshot section (contingency gauge, billed donut, 3-row budget-vs-actual bars, GM% sparkline, monthly-cost performance bar, cumulative-cash area trend) backed by a new `GET /api/project-scorecard-history`; allowlist-scoped office picker; route-handle filter scoping; project-detail header re-layout.
  - **Task-queue MCP server:** `update_task` extended with author / assignee operator-id edit fields (operator-table validation + activity-log entries); stale-read race on the pinned-task reorder UI patched.
  - Also drafted a reusable `security-checks` Claude Code skill covering an Azure stack (subscriptions / RBAC / Key Vault / SQL roles) + GitHub.
