...
>
> &middot; **Day-job, sprint across reporting / scorecards / team tooling.** Three parallel tracks shipping the same week.
>
> **Reporting tool:** a `Regenerate Report` flow (versioned `report_narrative_snapshots` + 30s undo + `View History` restore-to-version + chat-aware re-synthesis), embedded-chat persona lock with an admin-verb regression guard, PM questionnaire wired into the generated PDF, plus a PDF render cleanup pass (broken PM lookup, phantom photo rectangles, chat panel layout, `Clear Chat` destructive modal).
>
> **Scorecards dashboard:** a 6-chart Visual Snapshot section paired with the coding agent (contingency gauge, billed donut, budget-vs-actual bar group, GM% sparkline, monthly-cost performance bar, cumulative-cash area trend) backed by a new `GET /api/project-scorecard-history` endpoint; per-route handle-scoped office / status filters; allowlist-scoped office picker.
>
> **Team tooling:** an internal Claude Code plugin that distributes the team's full skill library to teammates' machines (installable, discoverable, clean-reinstall validated), plus a `mission-control-mcp` umbrella skill bundling the task-queue MCP toolkit and orchestrator ritual. **Stack across all three:** Python / FastAPI, TypeScript + React, SQL, Azure Container Apps with named-revision rollouts.
