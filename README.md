...
>
> &middot; **TR-300 v3.15.2**, fully published to crates.io with all 28 release assets across the four-installer Windows distribution + macOS + Linux, after a three-agent Opus 4.7 1M-context cross-platform audit landed 19 of 22 install / update / runtime findings (SHA256 verification of every MSI/EXE against the cargo-dist sidecar, atomic temp-then-rename rc-file writes, `command -v tr300` / `Get-Command` auto-run guards with a recursion sentinel, 5-second WMI timeouts, LC_ALL=C for `lscpu` / `lastlog` / `last`, 26 new unit tests) and a Linux clippy `unused_imports` fix-forward unblocked the cross-platform CI matrix
> &middot; **`shaughv_vintage`** project-level Claude Code automation install + mobile polish: `.claude/` + `.mcp.json` wiring context7 + chrome-devtools MCP, PostToolUse hooks for paired-changelog drift and Pretext `TEXT_REGISTRY` coupling, a `/changelog-pair` skill + registry-sync audit guide, motion-accessibility and design-system-guardian reviewer agents that travel with the repo, plus a hierarchical two-line mobile hero with the figurine revealed and anchored to viewport bottom, dimmed hero hairline, and `min-h-[100dvh]` extended to every breakpoint
> &middot; **realtime2_test**, OpenAI Realtime API voice-agent prototype gated behind explicit mic + audio-output device picks, with sage-painted required selectors, an AudioContext remote audio-level monitor that distinguishes "stream connected but silent" from "stream playing but inaudible", a draw/undraw SHAUGHV wordmark in the topbar shared with the personal site, and a refreshed `CLAUDE.md` archiving completed plans alongside the live Vercel deploy
...
|        | **May** | TR-300 v3.14.x preflight chain, v3.15.0/v3.15.1 four-installer Windows distribution + in-repo release skill, v3.15.2 cross-platform Opus 4.7 audit with SHA256 installer verification + atomic profile writes (28 assets, fix-forward green), Magic Pantry Phase H, shaughvOS v1.20, realtime2_test prototype + device-gated UX + SHAUGHV brandmark, shaughv_vintage refresh to SHAUGHV vintage design system + mobile hero polish + project-level Claude Code automation install (hooks, skills, agents, MCP) |
