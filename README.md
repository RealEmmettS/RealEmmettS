...
>
> &middot; **`shaughv-cdn`**, initial launch of a Cloudflare R2 brand-asset CDN at `cdn.shaughv.com` with 91 assets across one push: SHAUGHV wordmark (SVG + PNG fallbacks) + 4 favicon variants + 2 figurine SVGs and 10 webp variants, full Makira Sans-Serif (6 weights &times; OTF/TTF/WOFF/WOFF2 + variable axis) and full IBM Plex Mono (7 weights &times; normal+italic), vanilla + React/Framer Motion brandmark drop-ins; per-object Content-Type + Cache-Control set at upload (binaries immutable 1y, CSS/JS stale-while-revalidate), CORS open `*`, an auto-generated Live URL map regenerated via `scripts/generate-url-table.sh`, a paired CHANGELOG / HUMAN_CHANGELOG contract enforced in `CLAUDE.md`, and a project-level `/install-cloudflare-mcp` Claude Code skill that lets any session in the repo bootstrap the official Cloudflare MCP server
> &middot; **`shaughv-code`**, new Claude Code plugin marketplace consolidating every SHAUGHV skill into one editable plugin source-of-truth (`critical-thinking`, `openai-audio`, `pretext`, `perplexity-search`, `quiver-ai`, `shaughv-animated-brandmark`, `shaughv-design`), plus a v0.2.0 `/human-changelog` skill that translates technical CHANGELOGs into a paired plain-English HUMAN_CHANGELOG.md and installs a CLAUDE.md rule keeping the two in lockstep on every future commit
> &middot; **qrgen + QorkMe** vintage-palette adoption: `qrgen` rebuilt end-to-end as a SHAUGHV product surface with a dual-palette system (vintage cream by default, brutalist near-black + brand orange between 20:30 and 07:00 local time, set client-side by `public/boot.js` before first paint), self-hosted Makira + IBM Plex Mono, an asymmetric 12-col workspace grid, a new SegmentedControl primitive, a boundary-walking QR outline tracer powering a slower 6-phase boot loader, a hydration-clean inline-script + server-rendered BootSplash, and end-to-end AI generation through Gemini 3.1 flash image preview; [`QorkMe`](https://qork.me) recolored to the same SHAUGHV vintage palette (cream surfaces, sage as the single action color, olive body text, bamboo warm accent), IBM Plex Mono added to the mono slot, AmbientDecor + card 3D tilt deleted while the internal card shimmer beam is preserved
