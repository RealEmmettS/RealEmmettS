...
>
> &middot; **realtime2_test**, OpenAI Realtime API voice-agent prototype now gated behind explicit mic + audio-output device picks, with an AudioContext audio-level monitor, sage-painted required selectors, and a draw/undraw SHAUGHV wordmark in the topbar
> &middot; **TR-300 v3.15.1**, four-installer Windows distribution model live on crates.io with 28 release assets, a workflow_run-triggered Inno Setup + WiX pipeline, and a new in-repo release-workflow skill that encodes the end-to-end publish loop for future agents
> &middot; **`qube-machine-report-homepage`**, install panels switched to wrapper-script one-liners with per-installer TR-300 buttons, Windows MSVC auto-install preflight, and the TR-300 page reordered so the install panel anchors the bottom of the scroll
...
A growing ecosystem of Rust CLI diagnostics tools, all published to crates.io under canonical names. TR-300 just shipped v3.15.1 as the `tr300` crate ([`qube-machine-report`](https://github.com/QubeTX/qube-machine-report)) with a four-installer Windows distribution model live across all 28 release assets: Global MSI (perMachine, UAC), Corporate MSI (perUser, no UAC), Global EXE (Inno Setup, perMachine), Corporate EXE (Inno Setup, perUser, no UAC). Each writes an `HKCU\Software\TR300\InstallSource` registry marker so `tr300 update` reads the marker, dispatches to the matching installer for in-place upgrade, and never cross-fallbacks between installer types. The release pipeline pairs cargo-dist's `release.yml` for the Global MSI and shell installer scripts with a hand-authored `windows-installers.yml` that builds the three additional Windows installers, triggered on `workflow_run` off the Release workflow's completion to side-step the GITHUB_TOKEN loop-prevention gap, and the end-to-end publish loop is now codified in an in-repo `release` skill so future agents can drive and recover releases without rediscovery. The v3.14.x line that preceded it locked in the Windows preflight chain: a vswhere check that auto-installs MSVC Build Tools via winget, an execution-policy preflight, and a Display-formatted error path mapping AD / Intune / OneDrive / AppLocker / WDAC / antivirus / sharing-violation / storage-full / path-too-long scenarios to actionable advice. [`qube-network-diagnostics`](https://github.com/QubeTX/qube-network-diagnostics) (nd300 v3.0.x) ships hardened network-fix safety, an evidence-driven fix-loop with per-action stabilization windows, an automated crates-publish pipeline from `main`, and a cargo-first / installer-fallback self-update chain that cleans up non-cargo installs on upgrade. [`qube-system-diagnostics`](https://github.com/QubeTX/qube-system-diagnostics) (SD-300, republished as the canonical `tr300-tui` crate) has a stabilized updater and runner-compatible release metadata. Around the CLIs sit the web surfaces: [`QubeTX_Landing`](https://github.com/QubeTX/QubeTX_Landing) and [`qube-machine-report-homepage`](https://github.com/QubeTX/qube-machine-report-homepage) (install panels switched to wrapper-script one-liners across all platforms, per-installer TR-300 buttons for the four-installer Windows distribution, Windows MSVC auto-install preflight, a chained `tr300 install` that writes the shell-profile alias and auto-run line in one paste, and a Quote-above-Install reorder so the install panel anchors the bottom of the scroll), plus the multi-provider [SpeedQX](https://github.com/QubeTX/speedtest) web app (bootstrap CIs, inverse-variance weighted aggregation, RFC 3550 jitter) and offline installer bundles ([`qube-reports-executables`](https://github.com/QubeTX/qube-reports-executables)). Also where most freelance / client work lives.
...
[`realtime2_test`](https://github.com/RealEmmettS/realtime2_test) (a new OpenAI Realtime API voice-agent prototype on a hybrid Vercel-functions + Express architecture, with a "Cypher" persona auto-greet, paired mic / audio-output device selectors that gate Start Session and surface which physical input Chrome actually picked, sage-painted required-device selectors with hint copy that narrows as each is set, an AudioContext-driven remote audio-level monitor with silence-detection diagnostics, a visible audio-element pill at the viewport corner for Chrome autoplay-policy fallback, a draw/undraw SHAUGHV wordmark in the topbar shared with the personal site, a Lenis-smoothed fluid color-only transcript stream, and a per-route handler model that lets the same pure functions back both Express and Vercel deployments), web surfaces around the Qube TX ecosystem ([`QubeTX_Landing`](https://github.com/QubeTX/QubeTX_Landing), [`qube-machine-report-homepage`](https://github.com/QubeTX/qube-machine-report-homepage), [`qube-reports-executables`](https://github.com/QubeTX/qube-reports-executables) for offline installers), the [SpeedQX](https://github.com/QubeTX/speedtest) web speed-test app and its parallel Expo / React Native port (v2.0 technician-grade accuracy overhaul, network metadata, jitter breakdown, bootstrap CI, inverse-variance weighting), `shaughv_vintage` (vintage-leaning personal portfolio with a Pretext-powered responsive text layer and a consolidated nav + project-card a11y pass), a print-tuned [`resume-2026`](https://resume.emmetts.dev) on Makira / Personal-Vogue that auto-deploys via GitHub Pages, [Time](https://github.com/RealEmmettS/time) (atomic-clock alternative to time.gov with a Marzullo-uncertainty-based watch score), Remotion-based programmatic video experiments, MDX docs sites, and a rotating cast of small utilities (timer, qrgen, csv tools, countdown apps, movie list).
...
|        | **May** | TR-300 v3.15.0/v3.15.1 four-installer Windows distribution + in-repo release skill, Magic Pantry Phase H, shaughvOS v1.20, realtime2_test prototype + device-gated UX + SHAUGHV brandmark |
...
- **TR-300 Windows distribution.** v3.15.1 just shipped for the `tr300` crate: a four-installer Windows distribution model (Global MSI / Corporate MSI / Global EXE / Corporate EXE) with MSI/EXE-aware self-update that reads an `HKCU\Software\TR300\InstallSource` registry marker and dispatches `tr300 update` to the matching installer for in-place upgrades. cargo-dist's `release.yml` ships the Global MSI and shell installer scripts (22 assets); a hand-authored `windows-installers.yml` chained on `workflow_run` builds the three additional Windows installers via cargo-wix + Inno Setup for 6 more assets, for 28 total per release. The v3.14.x line that came just before it locked in the Windows preflight chain (vswhere + winget MSVC auto-install, execution-policy preflight, and a Display-formatted error advisor covering AD / Intune / OneDrive / AppLocker / WDAC / antivirus / sharing-violation / storage-full / path-too-long). The install one-liners now chain `tr300 install` so a single paste also writes the shell-profile alias and auto-run line. The end-to-end release workflow (pre-flight checks, version bumping, doc updates, local gates, optional Codex review, push, CI/crates-publish/release watch, tagging, post-release verification, and the fix-forward loop) is now codified as an in-repo `.agents/skills/release/SKILL.md` so future agents can drive and recover releases reliably without rediscovery.
