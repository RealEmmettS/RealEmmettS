...
>
> &middot; **Day-job, upstream-API rate-limit hardening + queued PDF round 2.** May closed with the editorial PDF redesign + symmetric server / client fixes (typeset PDF cover with custom-typeface wordmark via CSS `mask-image` over an SVG + text fallback, photo-frame treatment, 4-photo cap, editor header redesign with inline calendar SVG + data-freshness pill, cleared-photo-selection bug fixed in symmetry on both the server-side PDF and the client draft preview).
>
> **Latest, three PRs in review on the monthly-reporting tool.** Backend fail-fast on upstream 429 rate-limit responses paired with honest "temporarily unavailable" client messaging instead of a misleading "Photos unavailable"; a lazy-loaded photo drawer + cached photo list to cut upstream API volume at scale; and a paginated-render fix on the PDF so the signature / closing block never splits across a page break. All three authored by the coding agent on feature branches off the daily work-branch.
>
> **Queued next, PDF editorial round 2.** Download-PDF placeholder-resolution modal (resolve LLM square-bracket placeholders for recipient / owner name + ad-hoc fields before render), header-toolbar reflow when `Regenerate Report` is clicked, and a "go further" editorial pass on the PDF (cover band, gray section banners, KPI strip). Stack: Python + FastAPI, React 19 + TypeScript + Vite, hand-rolled CSS, Azure Container Apps + Static Web Apps with named-revision deploys per merge.
>
...
|        | **Jun** | **Day-job (in review):** upstream-API 429 rate-limit hardening on the monthly-reporting tool (backend fail-fast + frontend "temporarily unavailable" messaging, lazy-loaded photo drawer + cached photo list to cut request volume, no-page-break PDF signature / closing block). <br><br> **Queued:** Download-PDF placeholder-resolution modal, header-toolbar reflow on `Regenerate Report`, PDF editorial round 2 (cover band + gray section banners + KPI strip). |
...
- **Day-job, internal-tools cycle.** Python / FastAPI + TypeScript + React + SQL + Azure Container Apps.
  ...
  - **June rate-limit hardening (in review):** backend fail-fast on upstream 429 responses with honest "temporarily unavailable" client messaging, a lazy-loaded photo drawer + cached photo list to cut upstream request volume at scale, and a paginated-render fix so the PDF signature / closing block never splits across a page break. Queued behind it: Download-PDF placeholder-resolution modal (resolve LLM square-bracket placeholders for recipient / owner name + ad-hoc fields before render), header-toolbar reflow fix when `Regenerate Report` is clicked, and a "go further" editorial round 2 on the PDF (cover band + gray section banners + KPI strip).
