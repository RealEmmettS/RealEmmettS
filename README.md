...
>
> &middot; **Org-wide CI, security, and delivery hardening.** Migrated CI and deploys for roughly nine repositories off hosted runners onto a Docker-free, KEDA-scaled Azure Container Apps runner after shared minutes froze releases, then rolled a reusable advisory CI gate across the fleet with linting and an in-house secret scanner that replaced the third-party one. Added pre-push hooks and dual-review enforcement where branch protection is unavailable, moved deploys onto OIDC-federated managed identities instead of stored tokens, and made task completion an operator-only gate in the agent platform's MCP server so agents cannot self-approve their own work.
>
> &middot; **Native Rust CLI for the internal agent platform.** Matured the token-free terminal client into a ratatui triage cockpit with drill-down views and one-command dispatch that turns a task into a git worktree and launches a coding agent. Hardened it under a 30-agent adversarial review, persisted OAuth tokens with ETag optimistic concurrency so a container redeploy no longer forces re-login, and shipped self-updating Windows MSI installers past a files-in-use failure.
>
> &middot; **Internal data platform, gated API toward production.** Moved a FastAPI data gateway (managed identity to Azure SQL, Entra auth, keyset pagination, strict parameter allowlisting) toward retiring an internet-open SQL firewall rule, and wired Azure Monitor alerting for 5xx bursts, replica restarts, latency, and a zero-row guard on the view swap. Also root-caused stalled ingestion jobs and restored self-serve workbook refresh for a business owner who had no database principal.
