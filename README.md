...
>
> &middot; **qork `v1.1.1` + QorkMe, terminal-grade.** The Rust CLI ([`qork`](https://github.com/QubeTX/qork)) grew from a one-day `v1.0.0` into a self-managing tool: a four-installer Windows suite plus macOS `.pkg` / Linux `.deb` / `.rpm`, a `qork help` man-page command, a pre-shorten URL-liveness check that offline-rejects non-URLs and refuses dead links (`--no-check` to bypass), origin-aware `qork uninstall` across every channel, and a `qork update` that now prefers the prebuilt installer over a cargo rebuild.
>
> **QorkMe flipped back to canonical QubeTX:** the sage / bamboo sub-brand reverted to the blue &rarr; violet design system with a new "terminal" hero (`$ qork "url"`, blinking cursor, gradient wordmark), and an admin v2 console built on a single `admin_analytics()` RPC feeding a 14-day clicks / creation series, top-links ranking, and device breakdown as pure-CSS charts, over a debounced filterable / sortable links table.
>
> &middot; **shaughv-code `v0.15.0`, the plugin marketplace.** Four releases in a sprint: a `crystal-upscaler` image skill, a `usage-statusline` skill (live 5h + weekly usage %, burn-rate projection) with a zero-dep cross-platform installer, and the `shaughv-cdn` consumer skill rebuilt manifest-driven so it self-heals off `cdn.shaughv.com/tree.json` instead of hardcoding asset paths.
>
...
[`shaughv-code`](https://github.com/RealEmmettS/shaughv-code) is the SHAUGHV plugin marketplace at v0.15.0 after a sustained sprint: bundling `critical-thinking`, `openai-audio`, `pretext`, `perplexity-search`, `quiver-ai`, `shaughv-animated-brandmark`, `shaughv-design`, `/human-changelog`, `shaughv-cdn`, `naming-conventions`, and paired `gcs-storage` + `shaughv-gcs-storage`; two MCP server bundles (`@remotion/mcp` powering `/shaughv-code:create-video`, and `craft-docs` over Streamable HTTP + OAuth); plus a v0.8.0 dual-runtime split that ships a skills-only Codex plugin (`.codex-plugin/plugin.json` + `.agents/plugins/marketplace.json` + `AGENTS.md`) alongside the existing Claude Code surface so the same skill library installs on both runtimes. A v0.9.0 &rarr; v0.15.0 run then added 11 imported skills decoupled from the work stack, a `wb300` branch control-tower skill, a `crystal-upscaler` image skill (fal.ai Clarity), a `usage-statusline` skill (live 5h + weekly usage %, burn-rate projection) with a zero-dep cross-platform installer, and rebuilt the `shaughv-cdn` consumer skill manifest-driven so it self-heals off `cdn.shaughv.com/tree.json`.
...
- **shaughv-code v0.2.0 &rarr; v0.8.0.** Six-release sprint on the SHAUGHV plugin marketplace.
  ...
  - **v0.9.0 &rarr; v0.15.0:** 11 imported skills decoupled from the work stack; a `wb300` branch control-tower skill; a `crystal-upscaler` image skill (fal.ai Clarity); a `usage-statusline` skill (live 5h + weekly usage %, burn-rate projection) with a zero-dep cross-platform installer; and the `shaughv-cdn` consumer skill rebuilt manifest-driven so it self-heals off `cdn.shaughv.com/tree.json`.
