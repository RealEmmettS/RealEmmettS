...
>
> &middot; **Magic Pantry 2.0 / 2.0.1 store-ready release prep** on `magicPantry_2026`, now associated with its **EAS** project + account via `extra.eas.projectId` + `owner` in `app.json` so cloud builds + store submissions can run. Earlier in the window: bumped `app.json` versioning for store submission, added Supabase magic-link sign-in (passwordless `signInWithOtp` round-trip through `magicpantry://` deep links + a new `auth-email-actions` helper with unit coverage), normalized Expo local-testing config, and shipped a Neon &rarr; Supabase migration runner with backups so the prior Replit / Drizzle / Neon export imports cleanly without losing rows. **2.0.1**: stability pass on the realtime sync hook, a `recipe-sharing` helper module + tests, a 290-line `RecipesScreen` rewrite, and `StorePickerModal` / `RecipeDetailScreen` / `StoreListsScreen` / `PairingChips` polish. Same window codified an **AI model policy** in `AGENTS.md` (Haiku 4.5 reserved for lightweight item helpers `categorize-item` / `pair-suggestions` / `find-substitute`; Sonnet 4.6 mandated for all generation / parsing functions: `bulk-parse-items`, `generate-list`, `generate-recipe`, `import-recipe`, `recipes-from-list`) and the two Sonnet-backed edge functions were upgraded from Sonnet 4.5 to Sonnet 4.6 in lockstep. A new `.agents/demo-test-command.md` (mirrored into `CLAUDE.md`) pins the exact PowerShell-backgrounded `npx expo start` workflow for live Expo Go demos so agents stop substituting `npm start`, `expo start --tunnel`, or simulator-only commands when the user just wants a phone scan.
> &middot; **shaughv-code v0.2.0 &rarr; v0.7.0**, a five-release sprint on the SHAUGHV Claude Code plugin marketplace. **v0.3.0**: bundle the `@remotion/mcp` documentation server via `.mcp.json` and add a `/shaughv-code:create-video` slash command that scaffolds a Remotion Recorder project then adds `@remotion/web-renderer` inside it. **v0.4.0**: new `shaughv-cdn` consumer skill teaching agents how to consume `cdn.shaughv.com` (URL conventions, canonical `<link>` / `<script>` / `<img>` snippets, font preload patterns, the 1y-immutable vs 1d-SWR cache contract, CORS, license, decision matrix). **v0.5.0 / v0.5.1**: new `naming-conventions` skill (code identifiers, files / folders, git branches + commits, naming audit, rationale) plus an `npx skills` install path and a split README that gives consumers a copy-paste Update block separate from the maintainer's editing workflow. **v0.6.0**: paired `gcs-storage` (generic Google Cloud Storage reference) + `shaughv-gcs-storage` (pre-wired to `gs://shaughv` with bucket facts baked in: uniform IAM, public reads, 7-day soft delete, versioning on, CORS off), both smoke-tested upload / list / public-URL / delete / soft-delete-recovery against the live bucket before publishing. **v0.7.0**: bundle a second MCP server (`craft-docs`, Streamable HTTP, OAuth-gated on first tool use) so the agent can read from and write to a specific Craft Doc directly.
> &middot; **`italy-trip-video`** ships as a new Remotion project: a 50th-birthday Italy trip slideshow at 1920&times;1080 @ 30fps, 270.96s runtime, 92 chronological images, `divineSong.mp3` audio track, a `SubtitleOverlay` component with Makira font loading and an array of timed `lyricCaptions`, both an `ItalyTripSlideshow` and `ItalyTripSlideshowSubtitled` composition registered in `Root`, `@remotion/captions` wired in, build + still scripts for the subtitled output, and paired `CHANGELOG.md` + `HUMAN_CHANGELOG.md` with the standing dual-update rule installed in `CLAUDE.md` &sect;11. Pipeline: Remotion &rarr; CapCut for the final polish pass. Scaffolded by the same `/shaughv-code:create-video` slash command that landed in `shaughv-code` v0.3.0 the same week, the first production use of the SHAUGHV Remotion bundle.
...
Full rebuild of the Magic Pantry app, lifted out of the prior Replit + Express + Drizzle + Modelfarm stack and into the new `magicPantry_2026` codebase on Supabase (Postgres + Auth + RLS + Realtime) with Expo SDK 55 / React Native 0.83 / React 19.2 / React Navigation v7 / TanStack Query v5. AI features (item categorization on Claude Haiku 4.5, generation / parsing on Sonnet 4.6, URL recipe import via Firecrawl v2 + Sonnet 4.6, plus a service-role `delete-account` for App Store compliance) run through edge functions. **2.0 / 2.0.1 store-ready release prep** is in flight: bumped `app.json` for store submission, wired the app to its **EAS** project + account via `extra.eas.projectId` + `owner` so cloud builds + store submissions can run, added Supabase magic-link sign-in (passwordless `signInWithOtp` round-trip through `magicpantry://` deep links + a new `auth-email-actions` helper with unit coverage), normalized Expo local-testing config, and shipped a Neon &rarr; Supabase migration runner with backups so the prior Drizzle / Neon export imports cleanly without losing rows. 2.0.1 followed within hours: stability pass on the realtime sync hook, a `recipe-sharing` helper module + tests, a 290-line `RecipesScreen` rewrite, and `StorePickerModal` / `RecipeDetailScreen` / `StoreListsScreen` / `PairingChips` polish. A new `.agents/demo-test-command.md` (mirrored into `CLAUDE.md`) codifies the exact PowerShell-backgrounded `npx expo start` workflow for live Expo Go demos so agents don't substitute `npm start`, `expo start --tunnel`, or simulator-only commands when the user is asking for a phone scan. Same window codified an **AI model policy** in `AGENTS.md` reserving Haiku 4.5 for lightweight item helpers (`categorize-item`, `pair-suggestions`, `find-substitute`) and mandating Sonnet 4.6 for all generation / parsing functions (`bulk-parse-items`, `generate-list`, `generate-recipe`, `import-recipe`, `recipes-from-list`); the two Sonnet-backed edge functions were upgraded in lockstep. Sitting under the store-ready push is Phases I-L: smart entry, recipes library, substitutions, aisle-vs-manual sort. Long-pressing the + add button opens an iOS ActionSheet with "Paste a list" (multi-line modal &rarr; `bulk-parse-items` &rarr; editable confirmation list &rarr; bulk insert) and "Generate from prompt" (single-line &rarr; `generate-list` &rarr; same confirmation UI); a 500ms-debounced `PairingChips` row under the QuickAdd input runs `pair-suggestions` (Haiku) on every keystroke; the extended `categorize-item` returns canonicalized name + default quantity + category in one round-trip. The recipes table is back (migration 0009, jsonb arrays + `text[]` dietary tags + RLS own-only) with a Library / Generate / Import segmented control on `RecipesScreen`, a "What can I make?" pill that runs `recipes-from-list` against a chosen store and round-trips through `generate-recipe`, server-side dietary-tag inference on both `generate-recipe` and `import-recipe`, and `StorePickerModal`-backed "Add ingredients to list" that dedups via `addIngredientsToStore`. Long-press any item row &rarr; ActionSheet [Change category, Find substitute, Delete]; Find substitute calls `find-substitute` (Haiku) and surfaces a "name -- note" sheet that opens a Replace / Add as new picker. Migrations 0010 + 0011 add `store_lists.sort_mode` (default `aisle` grouped, opt-in `manual` flat), toggled owner-only in `StoreSettings`. Five new edge functions deployed (`bulk-parse-items`, `generate-list`, `pair-suggestions`, `recipes-from-list`, `find-substitute`); three updated; `tsc --noEmit` clean. Phase H still underneath: full account self-service (forgot / reset / change password, change username, signup-confirmation flow with 60s-cooldown resend) end-to-end via `magicpantry://` deep links for App Store launch, sharing tightened so non-owner projections drop emails entirely and `findUserByUsernameOrEmail` returns username + id only. RLS helpers live in a `private` schema, `citext` was moved out of `public`, and usernames are `citext` for case-insensitive uniqueness.
...
|        | **May** | TR-300 v3.14.x preflight chain, v3.15.0/v3.15.1 four-installer Windows distribution + in-repo release skill, v3.15.2 cross-platform Opus 4.7 audit with SHA256 installer verification + atomic profile writes (28 assets, fix-forward green), v3.15.3 closing F17 alias-collision warning + F20 windows-installers pre-flight (with `--repo` fix-forward) + F22 WMI worker-thread COMLibrary fix + Anthropic-vendored .claude/skills bundle (brainstorming / critical-thinking / architecture / system-design), v3.0.0 to v3.15.1 HUMAN_CHANGELOG backfill, Magic Pantry Phase H + Phases I-L (smart entry via long-press + ActionSheet + bulk-parse-items + generate-list + pair-suggestions, recipes library + What-can-I-make + dietary-tag inference, find-substitute, aisle/manual sort modes, 5 new edge functions) + 2.0/2.0.1 store-ready prep (magic-link sign-in, Neon&rarr;Supabase migration runner + backups, Expo local-testing config normalize, 2.0.1 stability pass on realtime sync hook + RecipesScreen rewrite + recipe-sharing helpers, AI model policy: Haiku 4.5 for item helpers / Sonnet 4.6 for generation, two edge functions upgraded to Sonnet 4.6, EAS Application Services wired into app.json for cloud builds + store submissions, .agents/demo-test-command.md pinning the PowerShell-backgrounded `npx expo start` workflow for live Expo Go demos), shaughvOS v1.20, realtime2_test prototype + device-gated UX + SHAUGHV brandmark, shaughv_vintage refresh to SHAUGHV vintage design system + mobile hero polish + project-level Claude Code automation install (hooks, skills, agents, MCP) + navigation a11y pass (IntersectionObserver scrollspy, dynamic aria-current, sage-tinted active links, logo top-scroll, min-h-[100dvh] hero across every breakpoint), shaughv-cdn launch (91 assets to Cloudflare R2 at cdn.shaughv.com + /install-cloudflare-mcp project skill), shaughv-code v0.2.0&rarr;v0.7.0 sprint (v0.3.0 @remotion/mcp + /create-video, v0.4.0 shaughv-cdn consumer skill, v0.5.0 naming-conventions, v0.5.1 split install/update README + npx skills path, v0.6.0 gcs-storage + shaughv-gcs-storage pre-wired to gs://shaughv, v0.7.0 craft-docs MCP server), italy-trip-video Remotion 50th-birthday slideshow (1920&times;1080@30fps, 270.96s, 92 images, divineSong.mp3, SubtitleOverlay with Makira + lyricCaptions, scaffolded by /create-video), qrgen rebuilt as a SHAUGHV product surface with a time-of-day dual-palette system + new QR outline tracer, QorkMe SHAUGHV vintage palette migration (cream / sage / olive / bamboo, IBM Plex Mono, AmbientDecor + card tilt removed) |
...
- **shaughv-code v0.2.0 &rarr; v0.7.0.** Five-release sprint on the SHAUGHV Claude Code plugin marketplace. **v0.3.0**: bundle the `@remotion/mcp` documentation server via `.mcp.json` and add a `/shaughv-code:create-video` slash command that scaffolds a Remotion Recorder project then adds `@remotion/web-renderer` inside it. **v0.4.0**: new `shaughv-cdn` consumer skill teaching agents how to consume `cdn.shaughv.com` (URL conventions for `/brand/`, `/fonts/`, `/js/`; canonical `<link>` / `<script>` / `<img>` snippets; font preload patterns; the 1y-immutable vs 1d-SWR cache contract; CORS; license restrictions; a fuzzy-ask decision matrix). **v0.5.0 / v0.5.1**: new `naming-conventions` skill (code identifiers, files / folders, git branches + commits, naming audit recipes, rationale), an `npx skills` install path, a consumer-facing Update block in the README, and the maintainer workflow renamed to "Editing a skill" with a pointer back to Update so consumers don't follow the wrong recipe. **v0.6.0**: paired `gcs-storage` (generic Google Cloud Storage reference covering install on macOS / Linux / Windows, ADC vs service-account auth, impersonation, upload / download / list / delete, flat vs HNS folders, public access, signed URLs and the 7-day cap gotcha, CORS, lifecycle, scripting flags, cheat-sheet, gotchas catalog; agent must ask for project ID + bucket before mutating commands) and `shaughv-gcs-storage` (pre-wired to `gs://shaughv` with bucket facts baked in: uniform IAM, public reads, 7-day soft delete, versioning on, CORS off, so the agent never has to ask), both smoke-tested upload / list / public-URL / delete / soft-delete-recovery against the live bucket before publishing. **v0.7.0**: bundle a second MCP server, `craft-docs`, over Streamable HTTP and OAuth-gated on first tool use, so the agent can read from and write to a specific Craft Doc directly (the committed URL is a Craft share link, not a credential). Underneath: legacy binary `.skill` bundles dropped, content normalized into a `skills/` tree, plugin manifest + marketplace entry under `.claude-plugin/`, `/human-changelog` skill that translates a repo's CHANGELOG.md into a plain-English HUMAN_CHANGELOG.md and installs a standing rule in CLAUDE.md so future agents keep the two in lockstep, paired `CHANGELOG.md` + `HUMAN_CHANGELOG.md` backfilled v0.1.0 through v0.7.0 with the standing Changelog rule in `CLAUDE.md`.
- **italy-trip-video.** New Remotion project: a 50th-birthday Italy trip slideshow at 1920&times;1080 @ 30fps, 270.96s runtime, 92 chronological images, `divineSong.mp3` audio track. `AGENTS.md` codifies the Remotion slideshow spec end-to-end (hard constraints, composition design, pipeline, asset conventions, optional light-leaks, skills, iteration guardrails). A `SubtitleOverlay` component loads Makira and renders an array of timed `lyricCaptions`; both an `ItalyTripSlideshow` and an `ItalyTripSlideshowSubtitled` composition are registered in `Root` so a subtitled render can be produced alongside the clean cut, with `@remotion/captions` added and build + still scripts wired for the subtitled output. Paired `CHANGELOG.md` + `HUMAN_CHANGELOG.md` backfilled from the existing five-commit history with the standing sync rule installed in `CLAUDE.md` &sect;11. Pipeline: Remotion &rarr; CapCut for the final polish pass. Scaffolded by the same `/shaughv-code:create-video` slash command that landed in `shaughv-code` v0.3.0 the same week, the first production use of the SHAUGHV Remotion bundle.
- **Magic Pantry 2.0 / 2.0.1 store-ready release prep.** App Store submission window. Bumped `app.json` for store submission, wired the app to its **EAS** (Expo Application Services) project + account via `extra.eas.projectId` + `owner = "realemmetts"` so cloud builds + store submissions can run, added Supabase magic-link sign-in (passwordless `signInWithOtp` round-trip through `magicpantry://` deep links, a new `auth-email-actions` helper with unit coverage, wired through `LoginScreen` + `AuthContext` + `App.tsx`), normalized Expo local-testing config, and shipped a Neon &rarr; Supabase migration runner with backups so the prior Drizzle / Neon export imports cleanly without losing rows. 2.0.1 followed within hours: a 712-line stability pass covering the realtime sync hook (48 / 33 split), a new `recipe-sharing` helper module + tests, a 290-line `RecipesScreen` rewrite, and polish on `StorePickerModal`, `RecipeDetailScreen`, `StoreListsScreen`, and `PairingChips`. Same window codified an **AI model policy** in `AGENTS.md` reserving `claude-haiku-4-5` for lightweight item helpers (`categorize-item`, `pair-suggestions`, `find-substitute`) and mandating `claude-sonnet-4-6` for all generation / parsing functions (`bulk-parse-items`, `generate-list`, `generate-recipe`, `import-recipe`, `recipes-from-list`); the `bulk-parse-items` and `generate-list` edge functions were upgraded from Sonnet 4.5 to Sonnet 4.6 in lockstep, and the policy explicitly forbids downgrading Sonnet-backed runtime code or edge functions to an older Sonnet model. The same window added a `.agents/demo-test-command.md` (mirrored into `CLAUDE.md`) codifying the exact live-demo workflow for showing the app on a real device: `npx expo start` run as a backgrounded PowerShell process with the QR code or `exp://` URL visible for Expo Go to scan, no `npm start` / `expo start --tunnel` / simulator-only substitutions allowed unless the user asks for them, so agents reaching for "let me just open the simulator" get rerouted to the actual phone-scan path users want for a live demo.
