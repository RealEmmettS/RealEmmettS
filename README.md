...
>
> &middot; **`shaughv_vintage`** navigation accessibility: an `IntersectionObserver`-driven scrollspy that tracks which section is currently in view and dynamically applies `aria-current="true"` plus sage-tinted active styling to the matching nav link, desktop + mobile menu links staying in lockstep as the user scrolls so screen-reader users get programmatic context for the single-page layout; the logo smooth-scrolls to the top of the page instead of mutating `#hero` in the URL, and a follow-on hero fix drops the `lg:` prefix from `min-h-[100dvh]` so mobile + tablet land on a full-viewport hero (the `100dvh` unit keeping iOS Safari's URL-bar collapse from pushing content out of view)
> &middot; **`shaughv-cdn`** + **`shaughv-code`** brand-system rails: the Cloudflare R2 brand-asset CDN at `cdn.shaughv.com` serving 91 versioned assets (SHAUGHV wordmark + figurines + favicons + the full Makira Sans-Serif and IBM Plex Mono families across every weight / italic / OTF / TTF / WOFF / WOFF2 axis, plus vanilla and React/Framer Motion brandmark drop-ins) with per-object Content-Type + Cache-Control set at upload and an auto-generated Live URL map regenerated by `scripts/generate-url-table.sh`; alongside it the SHAUGHV Claude Code plugin marketplace at v0.2.0, a single editable plugin source-of-truth bundling `critical-thinking`, `openai-audio`, `pretext`, `perplexity-search`, `quiver-ai`, `shaughv-animated-brandmark`, `shaughv-design`, and a new `/human-changelog` skill that translates technical CHANGELOGs into a paired plain-English HUMAN_CHANGELOG.md and installs a CLAUDE.md rule keeping the two in lockstep
> &middot; **qrgen + QorkMe** SHAUGHV vintage palette adoption: `qrgen` rebuilt end-to-end as a SHAUGHV product surface with a time-of-day dual-palette system (vintage cream by default, brutalist near-black + brand orange between 20:30 and 07:00 local time, set client-side by `public/boot.js` before first paint), self-hosted Makira + IBM Plex Mono, an asymmetric 12-col workspace grid, a new SegmentedControl primitive, a boundary-walking QR outline tracer powering a slower 6-phase boot loader, a hydration-clean inline-script + server-rendered BootSplash, and end-to-end AI generation through Gemini 3.1 flash image preview; [`QorkMe`](https://qork.me) recolored to the same SHAUGHV vintage palette (cream surfaces, sage as the single action color, olive body text, bamboo warm accent), IBM Plex Mono added to the mono slot, AmbientDecor + card 3D tilt deleted while the internal card shimmer beam is preserved
