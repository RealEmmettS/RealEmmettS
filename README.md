...
>
> &middot; **nd300 (`qube-network-diagnostics`), one-day reliability sprint.** A v3.0.8 &rarr; v3.0.11 run published to crates.io in a single day, hardening the network-diagnostics CLI around interruption, honesty, and bounded execution.
>
> **What landed:** every diagnostic bounded with timeouts + a Ctrl-C / watchdog drain and parallelized latency probes (v3.0.8), an interrupt-safe restore registry so a cancelled network-fix rolls back cleanly (v3.0.9), `build.rs` writes scoped to `OUT_DIR` + honest Windows uninstall / update paths (v3.0.10), and honest speed-test failure status + a real NDT7 throughput cap + non-blocking sysinfo (v3.0.11). Release CI now sends a User-Agent + retries crates.io API checks.
>
...
>
> **Rolling out strangler-fig:** parallel-run against the existing nightly batch to prove per-entity parity before retiring it to a weekly reconcile, with App Insights observability + low-false-positive alerting on the new tier. Stack: Python, Azure (Service Bus, Functions, Container Apps Jobs, Key Vault, App Insights), SQL on a medallion schema, all driven through the `az` CLI.
>
> &middot; **SHAUGHV plugin marketplace, v0.10.0.** [`shaughv-code`](https://github.com/RealEmmettS/shaughv-code) jumped two minor versions in a day: v0.9.0 imported 11 additional skills into the marketplace, then v0.10.0 decoupled those imported skills from the personal work stack so the library installs cleanly on any machine, on either the Claude Code or Codex runtime.
...
Around the CLIs: [`qube-network-diagnostics`](https://github.com/QubeTX/qube-network-diagnostics) (nd300 v3.0.11 after a same-day reliability sprint, interrupt-safe network-fix loop, cargo-first self-update), [`qube-system-diagnostics`](https://github.com/QubeTX/qube-system-diagnostics) (republished as `tr300-tui`), the multi-provider [SpeedQX](https://github.com/QubeTX/speedtest) web app, and the [`QubeTX_Landing`](https://github.com/QubeTX/QubeTX_Landing) + [`qube-machine-report-homepage`](https://github.com/QubeTX/qube-machine-report-homepage) marketing surfaces.
...
[`shaughv-code`](https://github.com/RealEmmettS/shaughv-code) is the SHAUGHV plugin marketplace at v0.10.0: bundling `critical-thinking`, `openai-audio`, `pretext`, `perplexity-search`, `quiver-ai`, `shaughv-animated-brandmark`, `shaughv-design`, `/human-changelog`, `shaughv-cdn`, `naming-conventions`, and paired `gcs-storage` + `shaughv-gcs-storage`; two MCP server bundles (`@remotion/mcp` powering `/shaughv-code:create-video`, and `craft-docs` over Streamable HTTP + OAuth); a v0.8.0 dual-runtime split that ships a skills-only Codex plugin (`.codex-plugin/plugin.json` + `.agents/plugins/marketplace.json` + `AGENTS.md`) alongside the Claude Code surface so the same library installs on both runtimes; and a v0.9.0 / v0.10.0 pass that imported 11 more skills and then decoupled those imported skills from the personal work stack so they install clean on any machine.
...
- **shaughv-code v0.2.0 &rarr; v0.10.0.** Sustained release sprint on the SHAUGHV plugin marketplace.
  ...
  - **v0.9.0:** imported 11 additional skills into the marketplace in one pass, widening the bundled library beyond the original SHAUGHV-authored set.
  - **v0.10.0:** decoupled those imported skills from the personal work stack so the library installs clean on any machine, with refreshed changelogs / README and `create-video` usage notes.
...
- **nd300 + SD-300.** `qube-network-diagnostics` v3.0.x hardened the network-fix loop with per-action stabilization windows and a cargo-first / installer-fallback self-update chain. `qube-system-diagnostics` republished as the canonical `tr300-tui` crate.
  - **v3.0.8 &rarr; v3.0.11 reliability sprint (one day):** every diagnostic bounded with timeouts behind a Ctrl-C / watchdog drain + parallelized latency probes (v3.0.8); an interrupt-safe restore registry so a cancelled network-fix rolls back cleanly (v3.0.9); `build.rs` writes scoped to `OUT_DIR` + honest Windows uninstall / update paths (v3.0.10); honest speed-test failure status + a real NDT7 throughput cap + non-blocking sysinfo (v3.0.11).
  - **Release CI:** crates.io API checks now send a User-Agent and retry on transient failures.
