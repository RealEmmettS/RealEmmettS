...
>
> &middot; **QubeTX_Landing `v3`, the studio site rebuilt from the ground up.** The [Qube TX](https://github.com/QubeTX/QubeTX_Landing) marketing site shipped a `v3.0.0` &rarr; `v3.2.0` redesign: a self-documenting `/design-system` page with a live terminal kit and a downloadable brand kit on a stable permalink, plus ScrollTrace and stat count-ups.
>
> **Underneath:** a `SYSTEM_INITIALIZER` first-load boot screen (dynamic readiness gating + a 5s minimum hold), a pure-math dot-grid wave field that replaced per-event ripple tweens, slot-roll per-character label motion, an LED dot-matrix panel filling the services grid, a restructured product line, and a hidden `/eggsy` key page with a downloadable cheatsheet.
>
> &middot; **shaughv-code `v0.18.2`, Codex-runtime parity + a reproducible packaging gate.** The SHAUGHV plugin marketplace ran from `v0.16.0` to `v0.18.2`: a new `ttdr` skill, the Codex integration brought to full parity with the Claude Code surface (packaged snapshot + bundled MCP), and a sync that decoupled four more skills from the work stack behind a new CI validate gate.
>
> **Reproducible + tidy:** the Codex package check is now byte-identical on Windows CI (`.mcp.json` pinned to LF) under a documented 1024-char skill-description cap, and QorkMe's `/install` page hardened its Windows one-liner to a full PowerShell `-ExecutionPolicy ByPass` wrapper.
>
...
>
> **Magic Pantry `2.0.2`:** offline-first durability + live collaborative sync. List items gained a server-set `updated_at` trigger, `REPLICA IDENTITY FULL`, and a `client_tag` idempotency key, so an at-least-once retry over a flaky connection can't duplicate or clobber a row. A per-row Realtime merge applies a collaborator's edit live without reverting the field you're mid-toggling (own-create echo dedup, delete-evict, newer-wins), and a one-sheet long-press item editor (quantity / recategorize / substitute / delete) lands alongside a Sonnet-backed Find Substitute that adds-and-crosses-off instead of replacing.
...
**TR-300 v3.17.0** ([`qube-machine-report`](https://github.com/QubeTX/qube-machine-report), `tr300` crate) adds migrate-cleanup: installing through a new channel (Windows installer, cargo, bare binary) detects and removes any prior install of a different kind, so the machine carries exactly one TR-300 at a time. It sits on **v3.16.0**, a seven-PR cross-platform stability pass: Windows + Linux collector accuracy, output / build robustness, install-update robustness, a self-update reliability deep-dive, and a test-coverage sweep.

Underneath, the v3.15.x hardening line that preceded it: the May Opus 4.7 1M-context cross-platform audit closed at v3.15.3 (WMI on a dedicated worker thread, a `windows-installers.yml` asset pre-flight, a `report`-alias collision warning), SHA256 verification of every downloaded MSI / EXE, atomic profile writes with a one-time backup, dual `powershell` + `pwsh` writes, and the four-installer Windows distribution (Global / Corporate &times; MSI / EXE) dispatched via an `HKCU\Software\TR300\InstallSource` registry marker.
...
`Next.js` `TypeScript` `Supabase` `Rust`

Custom aliases, click analytics, and a clean redirect layer on a Next.js + Supabase stack, with a terminal companion and a full admin console.

**Latest, the full QubeTX design system.** The sage / bamboo sub-brand accent reverted to canonical QubeTX blue &rarr; violet across every token, canvas LUT, and the focus ring; the home hero became a "terminal" composition (`$ qork "url"` mono line, blinking cursor, a Makira-Black gradient wordmark, live LED `MatrixClock`).

**Admin v2.** A single `admin_analytics()` RPC returns a 14-day clicks / creation series, top links, and a device breakdown in one round trip, rendered as pure-CSS bar charts above a debounced, filterable (ALL / ACTIVE / OFF / ALIAS), sortable links table with bulk actions; headline click metrics reconciled to the lifetime counter.

**qork CLI `v1.1.1`.** The terminal client surfaced on a new `/install` page with per-OS native-installer cards (the Windows one-liner now a full PowerShell `-ExecutionPolicy ByPass` wrapper); the public shorten API stayed agent-friendly (shared `createShortLink()`, `GET ?url=`, fully-qualified `href`) with `web` / `cli` / `api` source attribution into an admin breakdown card.
...
**Latest, offline-first durability + live merge (`2.0.2`).** List items got a server-set `updated_at` trigger, `REPLICA IDENTITY FULL`, and a `client_tag` idempotency key, so an at-least-once retry over a flaky connection can't duplicate or clobber a row (a Codex-flagged upsert clobber was swapped for insert-or-return-existing). A per-row Realtime merge now applies a collaborator's edit live without reverting the field you're mid-toggling (own-create echo dedup, delete-evict, newer-wins), and a one-sheet long-press editor handles name / quantity / AI recategorize / substitute / delete. Find Substitute moved to Sonnet 4.6, surfacing real errors and adding its pick as a new crossed-off item instead of replacing the original. Wraps the earlier `useStoreSyncStatus` offline queue that replays cleanly on reconnect.
...
[`shaughv-code`](https://github.com/RealEmmettS/shaughv-code) is the SHAUGHV plugin marketplace at v0.18.2 after a sustained sprint: bundling `critical-thinking`, `openai-audio`, `pretext`, `perplexity-search`, `quiver-ai`, `shaughv-animated-brandmark`, `shaughv-design`, `/human-changelog`, `shaughv-cdn`, `naming-conventions`, and paired `gcs-storage` + `shaughv-gcs-storage`; two MCP server bundles (`@remotion/mcp` powering `/shaughv-code:create-video`, and `craft-docs` over Streamable HTTP + OAuth); plus a v0.8.0 dual-runtime split that ships a skills-only Codex plugin (`.codex-plugin/plugin.json` + `.agents/plugins/marketplace.json` + `AGENTS.md`) alongside the existing Claude Code surface so the same skill library installs on both runtimes. A v0.9.0 &rarr; v0.15.0 run then added 11 imported skills decoupled from the work stack, a `wb300` branch control-tower skill, a `crystal-upscaler` image skill (fal.ai Clarity), a `usage-statusline` skill (live 5h + weekly usage %, burn-rate projection) with a zero-dep cross-platform installer, rebuilt the `shaughv-cdn` consumer skill manifest-driven so it self-heals off `cdn.shaughv.com/tree.json`, and at v0.16.0 added a full Mistral AI skill (OCR, audio transcription + TTS, chat / tools / structured output, embeddings, classifiers, batch, fine-tuning, agents); a `v0.17.0` &rarr; `v0.18.2` run then brought the Codex runtime to full parity with the Claude Code surface (packaged snapshot + bundled MCP), added a `ttdr` skill, decoupled four more skills from the work stack behind a new CI validate gate, and made the Codex package check byte-reproducible on Windows CI (`.mcp.json` LF pin, documented 1024-char description cap).

[`shaughv_vintage`](https://github.com/RealEmmettS/shaughv_vintage) is the vintage-leaning personal portfolio on the SHAUGHV vintage design system, now modernized onto the current framework majors (UI library, styling system, animation library, lint tooling) with a byte-identical font migration verified in-browser so the Pretext text-measurement layer never drifts. It carries Pretext-driven auto-fit headings, an `IntersectionObserver` scrollspy, a cursor-reactive paper dot-field hero (about 100 dots a frame, dormant when idle / off-screen), glyph-scramble nav + count-up section numbers, and a new `§ 03` self-drawing blueprint of the page itself: drafting paper, dimension lines that tick to the site's real values, type specimens, and the animation easing curve plotted live, assembling on scroll on desktop and drawing once on mobile, all reduce-motion aware. Sits on a removable project-level Claude Code automation pack (paired-changelog hook, Pretext `TEXT_REGISTRY` hook, `/changelog-pair` skill, motion-accessibility + design-system-guardian reviewer agents, MCP wiring for context7 + chrome-devtools).
...
**Qube TX surfaces.** [`QubeTX_Landing`](https://github.com/QubeTX/QubeTX_Landing) rebuilt to `v3.2.0`: a self-documenting `/design-system` page with a live terminal kit and a downloadable brand kit on a stable permalink, a `SYSTEM_INITIALIZER` boot screen (readiness gating + 5s hold), a pure-math dot-grid wave field, slot-roll per-character motion, an LED dot-matrix panel, and a hidden `/eggsy` key page. Plus [`qube-machine-report-homepage`](https://github.com/QubeTX/qube-machine-report-homepage), [`qube-reports-executables`](https://github.com/QubeTX/qube-reports-executables) (offline installers), the multi-provider [SpeedQX](https://github.com/QubeTX/speedtest) web app and its parallel Expo / React Native port carrying the v2.0 technician-grade accuracy overhaul (bootstrap CIs, inverse-variance weighting, RFC 3550 jitter, byte-weighted progress).
...
|        | **Jun** | **Data platform:** ingestion-optimization milestone underway, webhook-driven incremental tier (always-warm receiver + Service Bus + queue-scaled worker with idempotent `updated_at`-precedence upserts) replacing the nightly full batch, metadata-driven per-category polling cadence, App Insights observability + low-false-positive alerting, plus a per-project photo-links table and a multi-tenant cardinality + deploy-safety hardening pass. <br><br> **Reporting tool (shipped):** rate-limit hardening triplet (backend fail-fast on upstream 429s + honest "temporarily unavailable" UX, lazy-loaded photo drawer + cached photo list, page-break fix on the PDF signature block), invisible-form-field fix (phantom CSS token) + a Vitest undefined-token guard. <br><br> **In flight on `wb-2026-06-01`:** full-screen globe-loader on the three slowest actions, multi-select bulk-generate, last-report / last-edited sort, PDF editorial round 2 (cover band + gray section banners + KPI strip). <br><br> **TR-300:** `v3.16.0` seven-PR cross-platform stability + hardening pass (Windows / Linux collector accuracy, output / build + install-update robustness, self-update reliability deep-dive, test-coverage sweep), `v3.17.0` migrate-cleanup (one install per machine across installer / cargo / bare-binary channels). <br><br> **WB-300:** new Rust TUI workbranch orchestrator shipped to `v2.0.0` (real branch-hierarchy tree across every repo, attached-agent + lifecycle view, OS toast notifications, `wb300 help` / `uninstall`, `wb300.agent.v2` JSON). <br><br> **qork / QorkMe:** new cross-platform Rust URL-shortener CLI ([`qork`](https://github.com/QubeTX/qork)) `v1.0.0` &rarr; `v1.1.1` (full Windows MSI / EXE + macOS `.pkg` + Linux `.deb` / `.rpm` installer suite, `qork help` man pages, pre-shorten URL-liveness check, origin-aware `qork uninstall`, update path preferring the prebuilt binary over cargo); QorkMe `v5.0.0` &rarr; `v5.3.x` design-system flip from the sage / bamboo sub-brand back to canonical QubeTX blue &rarr; violet (terminal hero, admin v2 analytics console with `admin_analytics()` 14-day series + top links + device breakdown, filterable / sortable links table, `/install` page + agent-friendly shorten API + `web` / `cli` / `api` source attribution). <br><br> **Magic Pantry:** `2.0.1` offline-queue polish (optimistic-id alias retargeting, reconnect sequencing) + privacy / AI-rate-limit hardening, then `2.0.2` offline-first durability + live Realtime conflict-merge (server `updated_at` trigger, `REPLICA IDENTITY FULL`, `client_tag` idempotency), a one-sheet long-press item editor, and a Sonnet-backed Find Substitute with add-and-cross-off UX. <br><br> **shaughv-code:** `v0.9.0` &rarr; `v0.18.2` (11 imported skills + work-stack decoupling, `wb300` control-tower skill, `crystal-upscaler` image skill, `usage-statusline` skill + zero-dep cross-platform installer, manifest-driven `shaughv-cdn` rebuild, full Mistral AI skill, `ttdr` skill, full Codex-runtime parity with packaged snapshot + bundled MCP, a CI validate gate, byte-reproducible Windows packaging under a 1024-char description cap). <br><br> **QubeTX_Landing:** ground-up `v3.0.0` &rarr; `v3.2.0` studio-site rebuild (self-documenting `/design-system` page + terminal / downloadable brand kit on a stable permalink, `SYSTEM_INITIALIZER` boot screen, pure-math dot-grid wave field, slot-roll per-character motion, LED dot-matrix panel, `/eggsy` key page). <br><br> **shaughv_vintage:** modernized onto current framework majors (byte-identical font migration), cursor-reactive dot-field hero, glyph-scramble micro-interactions, and a `§ 03` self-drawing page blueprint (assemble-on-scroll desktop / draw-once mobile, reduce-motion aware). |
...
![Azure](https://img.shields.io/badge/Azure-204F20?style=flat-square&logoColor=F5E0C5)
...
- **QubeTX_Landing v3, the studio marketing site.** Ground-up `v3.0.0` &rarr; `v3.2.0` rebuild of the [Qube TX](https://github.com/QubeTX/QubeTX_Landing) site.
  - **Design system as a page:** a `/design-system` route documenting the system in place, a live terminal kit, and a downloadable brand kit on a stable permalink, with ScrollTrace + stat count-ups.
  - **Boot + motion:** a `SYSTEM_INITIALIZER` first-load boot screen with dynamic readiness gating and a 5s minimum hold; a pure-math dot-grid wave field that replaced per-event ripple tween bursts; slot-roll per-character label changes; an LED dot-matrix panel filling the services grid.
  - **Structure + a11y:** a restructured product line with a brand-font rendering fix, the mobile overlay portaled out of the header subtree, keyboard focus that follows in-page anchor navigation, and a hidden `/eggsy` key page with a downloadable cheatsheet.
...
- **shaughv-code v0.2.0 &rarr; v0.8.0.** Six-release sprint on the SHAUGHV plugin marketplace.
  ...
  - **v0.9.0 &rarr; v0.16.0:** 11 imported skills decoupled from the work stack; a `wb300` branch control-tower skill; a `crystal-upscaler` image skill (fal.ai Clarity); a `usage-statusline` skill (live 5h + weekly usage %, burn-rate projection) with a zero-dep cross-platform installer; the `shaughv-cdn` consumer skill rebuilt manifest-driven so it self-heals off `cdn.shaughv.com/tree.json`; and a full Mistral AI skill (OCR, audio transcription + TTS, chat / tools / structured output, embeddings, classifiers, batch, fine-tuning, agents) with dependency-optional Python runners.
  - **v0.17.0 &rarr; v0.18.2:** the Codex runtime brought to full parity with the Claude Code surface (packaged snapshot + bundled MCP); a new `ttdr` skill; a sync that decoupled four more skills from the work stack behind a new CI validate gate; and a byte-reproducible Codex package check on Windows CI (`.mcp.json` pinned to LF, a documented 1024-char skill-description cap, four over-cap descriptions trimmed with no context lost).
...
- **Magic Pantry, offline queued store actions.** 776-line addition on `magicPantry_2026` (12 files).
  ...
  - **2.0.2 durability + live merge:** server-set `updated_at` trigger + `REPLICA IDENTITY FULL` + `client_tag` idempotency so at-least-once retries can't duplicate or clobber (Codex-flagged upsert clobber swapped for insert-or-return-existing); a per-row Realtime field-preserving merge (own-create echo dedup, delete-evict, newer-wins); a one-sheet long-press item editor; Find Substitute moved to Sonnet 4.6 with surfaced errors + add-and-cross-off UX; raised AI output ceilings to stop large-output truncation.
...
- **qork CLI `v1.1.1` + QorkMe, full QubeTX design system.** The terminal companion matured into a self-managing tool while QorkMe flipped back to the canonical brand.
  - **qork CLI `v1.0.0` &rarr; `v1.1.1`:** cross-platform Rust CLI ([`qork`](https://github.com/QubeTX/qork)) that shortens from the terminal (`qork "url"`); full installer suite (Windows Global + Corporate MSI + EXE, macOS `.pkg`, Linux `.deb` / `.rpm`); `qork help` with man pages; a pre-shorten URL-liveness check (offline-rejects non-URLs, HEAD-pings to refuse dead links, `--no-check` bypass); origin-aware `qork uninstall` across every channel; and `qork update` preferring the prebuilt installer over a cargo rebuild.
  - **Design-system flip:** the sage / bamboo sub-brand accent reverted to canonical QubeTX blue &rarr; violet across tokens, the three canvas LUTs, the focus ring, and `::selection`; the home hero rebuilt as a "terminal" composition (`$ qork "url"` mono line, blinking cursor, Makira-Black gradient wordmark, live LED `MatrixClock`).
  - **Admin v2:** an `admin_analytics()` RPC (service-role only) returns a 14-day clicks / creation series, top links, and device breakdown in one round trip, drawn as pure-CSS bar charts; the links table gained debounced search, ALL / ACTIVE / OFF / ALIAS filters, sortable columns, and bulk purge; headline click metrics reconciled to the lifetime counter.
  - **Site + API:** `/install` page with per-OS installer cards, branded `install.sh` / `install.ps1`, a `public/llms.txt` agent guide; the public shorten API stayed agent-friendly (`createShortLink()`, `GET ?url=`, fully-qualified `href`) with `web` / `cli` / `api` source attribution into an admin breakdown card.
...
- **TR-300 v3.16.0 &rarr; v3.17.0, stability + single-install.** Post-audit hardening on [`qube-machine-report`](https://github.com/QubeTX/qube-machine-report).
  - **v3.16.0:** a seven-PR cross-platform stability pass covering output / build robustness, cross-platform frequency + codename, Windows and Linux collector accuracy, install / update robustness, a self-update reliability deep-dive, and a dedicated test-coverage sweep.
  - **v3.17.0:** migrate-cleanup. Installing through any channel (Windows installer, cargo, bare binary) detects and removes a prior install of a different kind, so exactly one TR-300 lives on the machine at a time.
...
- **shaughv_vintage + repo-travelling Claude Code automation.** Vintage-leaning portfolio refresh + a removable, in-repo automation pack.
  ...
  - **June modernization:** framework majors upgraded behind a byte-identical font migration verified in-browser; a cursor-reactive paper dot-field hero, glyph-scramble + magnetic micro-interactions, and a `§ 03` self-drawing blueprint of the page (dimension lines, type specimens, easing-curve plot) that assembles on scroll on desktop and draws once on mobile, all reduce-motion aware; performance pass killed per-pixel cursor re-renders and off-screen blur work.
...
- **Day-job, internal-tools cycle.** Python / FastAPI + TypeScript + React + SQL + Azure (Service Bus / Functions / Container Apps).
  - **Central Data Platform, ingestion optimization (June):** re-architecting a uniform nightly full-batch sync from an upstream SaaS into a webhook-driven incremental tier. Always-warm receiver validates a shared-secret `Authorization` header and enqueues a thin envelope to Service Bus; a KEDA-scaled Container Apps Job worker dedups, hydrates the full record by id, and idempotently upserts with `updated_at` precedence (safe under at-least-once / out-of-order delivery). Per-entity polling is driven by a metadata cadence config table so each category is fetched only as often as its freshness target needs, paced under a dual-bucket rate budget. Rolled out strangler-fig: parallel-run vs the nightly batch to prove parity, then retire it to a weekly reconcile. App Insights observability + windowed low-false-positive alerting; identity / RBAC bound through a single owner via the `az` CLI. Plus a per-project photo-links table (links + metadata, no blobs) and a hardening sweep, multi-tenant cardinality fix, reconciled drifting sync copies, `--revision-suffix` deploy-safety.
  ...
  - **June rate-limit hardening (landed):** backend fail-fast on upstream 429 responses with honest "temporarily unavailable" client messaging, a lazy-loaded photo drawer + cached photo list to cut upstream request volume at scale, and a paginated-render fix so the PDF signature / closing block never splits across a page break.
  - **June form-field fix (landed):** invisible questionnaire date-picker + photo-selector inputs traced to a phantom CSS custom property (an undefined token with no fallback invalidating the whole `border` declaration); swapped all references to sanctioned tokens and added a Vitest guard that fails if the stylesheet references any undefined custom property, plus a code-review follow-up correcting the guard's own token advice.
  - **June work-branch (in flight):** full-screen globe-loader animation wired into the three slowest user actions (generate / regenerate / download); multi-select bulk-generate drafts from the home list; last-report day + last-edited sort on the home project list (backend half landed, frontend half pending); draft-editor header-toolbar reflow on `Regenerate Report` (routed through a portaled generic confirm component); PDF editorial round 2 (cover band + gray section banners + KPI strip).
