...
>
> &middot; **qork, a CLI for the QorkMe shortener.** A brand-new cross-platform Rust CLI ([`qork`](https://github.com/QubeTX/qork)) shipped to `v1.0.0` in a day: shorten a URL straight from the terminal (`qork "url"`), with the full TR-300 installer suite ported across (WiX / MSI build, Windows Global + Corporate MSI + EXE, macOS `.pkg` for Apple Silicon + Intel, Linux `.deb` / `.rpm` for x86_64 + ARM64).
>
> **QorkMe `v5.3.1`** then surfaced it on the site: a new `/install` page with per-OS native-installer cards, a recommended one-liner install, an agent-friendly public shorten API (shared `createShortLink()`, a `GET ?url=` mode, a fully-qualified `href` on every response), and creation-source attribution (`web` / `cli` / `api`) wired through the RPC into a new admin breakdown card.
>
> &middot; **WB-300, the workbranch orchestrator.** A Rust TUI ([`qube-workbranch-view`](https://github.com/QubeTX/qube-workbranch-view)) at `v2.0.0` that renders the real git branch hierarchy (repo &rarr; trunk &rarr; workbranch &rarr; task) across every active repo at once, each row carrying its attached coding agent, lifecycle stage, worktree path, and the files it is editing right now, with native OS toasts on commit / push / conflict-risk and a `wb300.agent.v2` JSON contract exposing the hierarchy to other tools.
>
...
Custom aliases, click analytics, and a clean redirect layer on a Next.js + Supabase stack, now with a terminal companion.

**Latest `v5.3.1`, the qork CLI.** A brand-new cross-platform Rust CLI ([`qork`](https://github.com/QubeTX/qork)) shortens URLs straight from the terminal and ships the full TR-300 installer suite ported across (WiX / MSI, Windows Global + Corporate MSI + EXE, macOS `.pkg` Apple Silicon + Intel, Linux `.deb` / `.rpm` x86_64 + ARM64). The site gained an `/install` page with per-OS native-installer cards, branded `install.sh` / `install.ps1` wrappers, and a `public/llms.txt` agent guide.

**Underneath:** the public shorten API went agent-friendly (shared `createShortLink()`, a `GET ?url=` mode, a fully-qualified `href` on every response) and every link now records its creation source (`web` / `cli` / `api`) through the RPC into a new admin "New links by source" card.

**`v5.0.0` rebuild (earlier in June):** every page rebuilt on the QubeTX structural design language: a Matrix identity recolored to the QorkMe sage &rarr; bamboo ramp with a live LED `MatrixClock`, `LoadSequence` entrance choreography, slot-roll status interactions, a single-RPC admin console, a six-migration Supabase sync, and Lighthouse 100 accessibility / 100 best-practices.
...
- **qork CLI + QorkMe `v5.3.1`.** A new terminal companion for the QorkMe shortener, plus the agent-friendly API and source attribution that surface it.
  - **qork CLI:** brand-new cross-platform Rust CLI ([`qork`](https://github.com/QubeTX/qork)) at `v1.0.0` that shortens straight from the terminal (`qork "url"`); the full TR-300 installer suite ported across (WiX / MSI, Windows Global + Corporate MSI + EXE, macOS `.pkg` Apple Silicon + Intel, Linux `.deb` / `.rpm` x86_64 + ARM64).
  - **Site integration (`v5.3.1`):** an `/install` page with per-OS native-installer cards, branded `install.sh` / `install.ps1` wrappers, a `public/llms.txt` agent guide, the public shorten API made agent-friendly (`createShortLink()`, `GET ?url=`, fully-qualified `href`), and creation-source attribution (`web` / `cli` / `api`) through the RPC into a new admin breakdown card.
  - **`v5.0.0` rebuild identity:** Matrix components recolored to the QorkMe sage &rarr; bamboo ramp, `dotFont` extended with digits + colon, a new anime-driven live LED `MatrixClock`; home gains `LoadSequence` entrance choreography, slot-roll status interactions, and a sage dot-field pulse on link creation.
