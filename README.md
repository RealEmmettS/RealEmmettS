...
>
> &middot; **shaughv-code `v0.16.0` + the qork install surface.** The SHAUGHV marketplace reached a Mistral-capable `v0.16.0`: a skill covering OCR, audio transcription + text-to-speech, chat / tools / structured output, embeddings, classifiers, batch, fine-tuning, and agents. Alongside, QorkMe's `/install` page matured: native per-OS installers surfaced under the command, slot-roll Windows download buttons, and an agent-friendly shorten API feeding `web` / `cli` / `api` source attribution into admin v2.
>
...
**qork CLI `v1.1.1`.** The terminal client surfaced on a new `/install` page with per-OS native-installer cards; the public shorten API stayed agent-friendly (shared `createShortLink()`, `GET ?url=`, fully-qualified `href`) with `web` / `cli` / `api` source attribution into an admin breakdown card.
