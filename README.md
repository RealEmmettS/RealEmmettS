...
>
> &middot; **Native Rust CLI for the internal agent platform.** Matured the token-free terminal client past its first milestone: full parity across the server's tool surface, write and lifecycle commands behind a validator layer that rejects malformed writes before the network, a ratatui triage cockpit with drill-down levels, and one-command dispatch that turns a task into a git worktree and launches a coding agent. It rides a three-OS CI grid with a supply-chain gate; real installers (Windows MSI/EXE, macOS DMG, Linux AppImage) self-update unattended and publish to Azure Blob Storage behind a signed checksum manifest.
>
> &middot; **Internal data platform, reliability and gated access.** Root-caused a reporting-snapshot pipeline that had silently failed for weeks, a duplicate source row fanning a SQL view into a MERGE primary-key violation that also killed the job's own error logging, then fixed it with row-number deduplication, transaction-state-aware logging, and owner-context stored procedures. On the access side, a FastAPI data-gateway (managed identity to Azure SQL, keyset pagination, strict allowlisting) now refreshes a workbook over Entra identity, and per-identity Entra app-role authorization is proven on a shared data endpoint, with one live unauthenticated data-browse endpoint closed in the same pass.
>
...
- **Agent-orchestration tooling.** Native Rust CLIs (two-crate Cargo workspaces) for an internal MCP task-orchestration platform, CI-gated from the first commit: formatting, linting, a three-OS test grid, an MSRV job, and a supply-chain gate. Between them they carry OAuth 2.0 / PKCE auth with OS-keyring tokens, a hand-rolled streaming HTTP/SSE MCP transport verified against a live server, full parity across the server's tool surface, a client-side validator of roughly thirty rules that rejects malformed writes before the network, a hermetic harness that boots a throwaway server for true end-to-end tests, a numpad-driven TUI triage cockpit, and a one-command dispatch that prepares a git worktree and launches a coding agent. Cross-platform installers build in CI, self-update unattended on a per-user MSI channel, and publish to Azure Blob Storage behind a signed checksum manifest via federated identity (OIDC).
