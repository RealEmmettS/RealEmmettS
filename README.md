...
>
> &middot; **TR-300 v3.15.1**, four-installer Windows distribution model live on crates.io with 28 release assets and a workflow_run-triggered Inno Setup + WiX pipeline
> &middot; **realtime2_test**, OpenAI Realtime API voice-agent prototype on a hybrid Vercel-functions + Express architecture, deployed live
> &middot; **`qube-machine-report-homepage`**, refreshed install panels with wrapper-script one-liners, per-installer TR-300 buttons, and Windows MSVC auto-install preflight
...
A growing ecosystem of Rust CLI diagnostics tools, all published to crates.io under canonical names. TR-300 just shipped v3.15.1 as the `tr300` crate ([`qube-machine-report`](https://github.com/QubeTX/qube-machine-report)) with a four-installer Windows distribution model live across all 28 release assets: Global MSI (perMachine, UAC), Corporate MSI (perUser, no UAC), Global EXE (Inno Setup, perMachine), Corporate EXE (Inno Setup, perUser, no UAC). Each writes an `HKCU\Software\TR300\InstallSource` registry marker so `tr300 update` reads the marker, dispatches to the matching installer for in-place upgrade, and never cross-fallbacks between installer types. The release pipeline pairs cargo-dist's `release.yml` for the Global MSI and shell installer scripts with a hand-authored `windows-installers.yml` that builds the three additional Windows installers, triggered on `workflow_run` off the Release workflow's completion to side-step the GITHUB_TOKEN loop-prevention gap. The v3.14.x line that preceded it locked in the Windows preflight chain: a vswhere check that auto-installs MSVC Build Tools via winget, an execution-policy preflight, and a Display-formatted error path mapping AD / Intune / OneDrive / AppLocker / WDAC / antivirus / sharing-violation / storage-full / path-too-long scenarios to actionable advice. [`qube-network-diagnostics`](https://github.com/QubeTX/qube-network-diagnostics) (nd300 v3.0.x) ships hardened network-fix safety, an evidence-driven fix-loop with per-action stabilization windows, an automated crates-publish pipeline from `main`, and a cargo-first / installer-fallback self-update chain that cleans up non-cargo installs on upgrade. [`qube-system-diagnostics`](https://github.com/QubeTX/qube-system-diagnostics) (SD-300, republished as the canonical `tr300-tui` crate) has a stabilized updater and runner-compatible release metadata. Around the CLIs sit the web surfaces: [`QubeTX_Landing`](https://github.com/QubeTX/QubeTX_Landing) and [`qube-machine-report-homepage`](https://github.com/QubeTX/qube-machine-report-homepage) (refreshed install panels with wrapper-script one-liners across all platforms, per-installer TR-300 buttons, Windows MSVC auto-install preflight, and a chained `tr300 install` that writes the shell-profile alias and auto-run line in one paste), plus the multi-provider [SpeedQX](https://github.com/QubeTX/speedtest) web app (bootstrap CIs, inverse-variance weighted aggregation, RFC 3550 jitter) and offline installer bundles ([`qube-reports-executables`](https://github.com/QubeTX/qube-reports-executables)). Also where most freelance / client work lives.
...
|        | **May** | TR-300 v3.15.0/v3.15.1 four-installer Windows distribution, Magic Pantry Phase H, shaughvOS v1.20, realtime2_test prototype |
